...
session_start();	//세션시작
	...
	if (!isset($_SESSION['uid'])) {	//세션 설정이 되어 있지 않고
		if (isset($_POST['submit'])) {	//index.html에서 login 버튼이 눌러졌다면..
			$_SESSION['uid']=$id;	//세션설정
		
			if (($id == 'admin') and ($pass == '1111')) {	//관리자 로그인
				header("Location:http://localhost/proj/session/admin.php");	//자동으로 링크 넘어감
			}
			else {	//학생 로그인
				...
				if (mysqli_num_rows($result) == 1) {	//성공 
					...
					header ("Location:http://localhost/proj/session/register.php");	//수강신청 화면으로 넘어감
				}
				else {
